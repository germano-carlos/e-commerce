...
class Operacao {
    ...
    async SalvarConfiguracao(parametros, callbackSuccess, callbackError) {
        try {
            let ret = await request("POST", `${this.endpoints.workAPI}/Operacao/SalvarConfiguracoesProduto`, parametros, null, 'gerencial');
    ...
    async ListarConfiguracoes(callbackSuccess, callbackError) {
        try {
            let ret = await request("POST", `${this.endpoints.workAPI}/Operacao/ListarConfiguracoesProduto`, [], null, "gerencial");
    ...
    async listarTagsPorCaracter(parametros, callbackSuccess, callbackError) {
        ...
        try {
            let ret = await request("POST", `${this.endpoints.workAPI}/Operacao/ListarTags`, form, null, 'gerencial');
            if (ret && ret.status == 200 && callbackSuccess) {
                callbackSuccess(ret.data)
            } else if (!ret || (ret.status != 200 && callbackError)) {
                callbackError()
    ...
    async ListarEnderecosPorUserId(parameters, callback) {
        let data = await request("POST", `${this.endpoints.workAPI}/Operacao/enderecos`, null, parameters);
        if (callback) {
            callback();
        }
    ...
    async listarProdutos(parametros, callbackSuccess, callbackError) {
        try {
            let ret = await request("POST", `${this.endpoints.workAPI}/Operacao/ListarProdutos`, null, null, 'gerencial');
            if (ret && ret.status == 200 && callbackSuccess) {
                callbackSuccess(ret.data)
            } else if (!ret || (ret.status != 200 && callbackError)) {
                callbackError()

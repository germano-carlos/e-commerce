<template>
    <div>
        <div class="modal-backdrop" v-if="isQuickViewOpen && this.product"></div>
        <transition name="slide-fade">
            <!-- Start Products QuickView Modal Area -->
            <div v-if="isQuickViewOpen && this.product" class="modal fade productQuickView" id="productQuickView" tabindex="-1" role="dialog" aria-hidden="true">
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="product-content">
                                    ...
                                    <div class="price">
                                        <span class="new-price">R$ {{this.product.price}}</span>
                                    </div>
                                    ...
                                    <a :href="`/products-details/${this.product.id}`" class="view-full-info">Ver informações completas</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        increaseQuantity(){
            if(this.quantity > 9){
                this.$toast.error("Não é possível adicionar mais de 10 itens",{
                    icon: 'fas fa-cart-plus'
                });
            } else {
                this.quantity++
        ...
        decreaseQuantity() {
            if(this.quantity < 2){
                this.$toast.error("Não é possível adicionar menos de 1 item",{
                    icon: 'fas fa-cart-plus'
                });
            } else {
                this.quantity--;
